...
public class ListTester {
    ...
    /**
     * Valid command line args include: -a : print results from all tests
     * (default is to print failed tests, only) -s : hide Strings from
     * toString() tests -m : hide section summaries in output
     *
     * @param args not used
     */
...
//			break;
     ...
     *
     * @return [B,A] after addToFront(B,A)
     */
    private IndexedUnsortedList<Integer> A_addToFrontB_BA() {
        IndexedUnsortedList list = emptyList_addToFrontA_A();
    ...
    private IndexedUnsortedList<Integer> AB_IterRemoveA_B() {
        IndexedUnsortedList<Integer> list = A_addB_AB(); // Create a list with [A, B]
        Iterator<Integer> it = list.iterator();
        it.next();
        it.remove();
        return list;
	...
	private void testTwoElementList(Scenario<Integer> scenario, String scenarioName, Integer[] contents, String contentsString) {
        ...
        try {
            ...
            printTest(scenarioName + "_testRemove" + contentsString.charAt(0), testRemoveElement(scenario.build(), contents[1], Result.MatchingValue));
            printTest(scenarioName + "_testRemoveNoIndex" + contentsString.charAt(1), testRemoveElement(scenario.build(), contents[1], Result.MatchingValue));
            printTest(scenarioName + "_testRemoveNoIndexX", testRemoveElement(scenario.build(), ELEMENT_X, Result.NoSuchElement));
            printTest(scenarioName + "_testFirst", testFirst(scenario.build(), contents[0], Result.MatchingValue));
	...
	private void testThreeElementList(Scenario<Integer> scenario, String scenarioName, Integer[] contents, String contentsString) {
        ...
        try {
            ...
            printTest(scenarioName + "_testRemoveNoIndex" + contentsString.charAt(1), testRemoveElement(scenario.build(), contents[2], Result.MatchingValue));
            printTest(scenarioName + "_testRemoveNoIndex2" + contentsString.charAt(2), testRemoveElement(scenario.build(), contents[2], Result.MatchingValue));
            printTest(scenarioName + "_testRemoveNoIndexX", testRemoveElement(scenario.build(), ELEMENT_X, Result.NoSuchElement));
            printTest(scenarioName + "_testFirst", testFirst(scenario.build(), contents[0], Result.MatchingValue));
            printTest(scenarioName + "_testLast", testLast(scenario.build(), contents[2], Result.MatchingValue));
            printTest(scenarioName + "_testContains0" + contentsString.charAt(0), testContains(scenario.build(), contents[1], Result.True));
            printTest(scenarioName + "_testContains1" + contentsString.charAt(1), testContains(scenario.build(), contents[1], Result.True));
            ...
            printTest(scenarioName + "_iterNextNextRemove_testIterRemove", testIterRemove(iterAfterRemove(iterAfterNext(scenario.build(), 2)), Result.IllegalState));
			printTest(scenarioName + "_iterNextNextRemoveNext_testIterHasNext", testIterHasNext(iterAfterNext(iterAfterRemove(iterAfterNext(scenario.build(), 2)),1), Result.False));
            printTest(scenarioName + "_iterNextNextRemoveNext_testIterNext", testIterNext(iterAfterNext(iterAfterRemove(iterAfterNext(scenario.build(), 2)), 1), null, Result.NoSuchElement));
            printTest(scenarioName + "_iterNextNextRemoveNext_testIterRemove", testIterRemove(iterAfterNext(iterAfterRemove(iterAfterNext(scenario.build(), 2)), 1), Result.NoException));
    ...
    /**
     * Runs list's iterator next() method and checks result against
     * expectedResult
     *
     * @param iterator an iterator already positioned for the call to hasNext()
    ...
    /**
     * Runs list's iterator remove() method and checks result against
     * expectedResult
     *
    ...
    /**
     * Runs ListIterator previous() method and checks result against
     * expectedResult
     *
     * @param iterator an iterator already positioned for the call to
     * hasPrevious()
    ...
    /**
     * Runs ListIterator previousIndex() and checks result against expected
     * Result
     *
    ...
    /**
     * Runs listIterator() method twice on a given list and checks result
     * against expectedResult
     *
     * @param list a list already prepared for a given change scenario
     * @param expectedResult
     ...
     * backed up numCallsToPrevious times.
     *
     * @param iterator
    ...
    /**
     * Helper for testing ListIterators. Return a ListIterator that has had
     * remove() called once.
     *
     * @param iterator
     * @return same Iterator following a call to remove()
